...
from utils.odswriter import ods_components
from utils.odswriter.formula import Formula
...
class ODSWriter(object):
    ...
    def __init__(self, odsfile, compression=ZIP_STORED):
        ...
        self.zipf.writestr(
            "META-INF/manifest.xml", ods_components.manifest_xml.encode("utf-8")
        )
...
class Sheet(object):
    def __init__(self, dom, name="Sheet 1", cols=None):
        ...
        if self.cols is not None:
            ...
            col.setAttribute("table:number-columns-repeated", str(self.cols))
    ...
    def writerow(self, cells):
        ...
        for cell_data in cells:
            ...
            if isinstance(cell_data, (datetime.date, datetime.datetime)):
                ...
                cell.setAttribute(
                    "office:time-value", cell_data.strftime("PT%HH%MM%SS")
                )
                ...
                cell.setAttribute(
                    "office:boolean-value", "true" if cell_data else "false"
                )
                cell.setAttribute("table:style-name", "cBool")
                text = "TRUE" if cell_data else "FALSE"

            elif isinstance(cell_data, (float, int, decimal.Decimal)):
                cell.setAttribute("office:value-type", "float")
                float_str = str(cell_data)
